...
contains
	...
	! Nonlinear problem (for SIMULTECH 2011, the third experiment; for Int. J for Numerical Methods in Engineering, the fourth experiment; NOVA Publishers, the fourth experiment; WCE2013, first experiment)
	fRHS = -the_am * myu_factor * theT(omega1, theta, kappa, the_am, the_add, time) ** (alpha - 1) * cos(time) ** 2.0 * (the_am * alpha * cosphi * cos(time) ** 2.0 * (omega1 ** 2.0 * cos(omega1 * xs + theta * cos(kappa * ys) * sin(time)) ** 2.0 + sinphi * sin(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * (kappa * theta * sin(time) * cos(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * sin(kappa * ys) * cosphi + sin(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * sinphi)) + &
		   theT(omega1, theta, kappa, the_am, the_add, time) * sin(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * (sinphi ** 2.0 - cosphi ** 2.0 - omega1 ** 2.0) + & 
		   kappa * theta * sin(time) * cosphi * (alpha * the_am * cos(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * sin(kappa * ys) * cosphi * cos(time) ** 2.0 * (kappa * theta * sin(time) * cos(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * sin(kappa * ys) * cosphi + sin(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * sinphi) - theT(omega1, theta, kappa, the_am, the_add, time) * (kappa * theta * sin(time) * sin(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * sin(kappa * ys) ** 2.0 * cosphi + cos(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * (kappa * cos(kappa * ys) * cosphi - 3.0 * sin(kappa * ys) * sinphi)))) / & 
		   R_Earth ** 2.0 / spread(mycosTabX, 1, lx) + the_am * cosphi * (cos(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * cos(time) ** 3.0 * theta * cos(kappa * ys) - sin(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * sin(2.0 * time))
...
!!!!!!!
	...
	! Nonlinear problem (for SIMULTECH 2011, the third experiment; for Int. J for Numerical Methods in Engineering, the fourth experiment; NOVA Publishers, the fourth experiment; WCE2013, first experiment)
	theT = the_am * sin(omega1 * xs + theta * cos(kappa * ys) * sin(time)) * cosphi * cos(time) ** 2.0 + the_add
